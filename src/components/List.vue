...
export default {
  /**
   * 10. Не совсем понятно для чего в компоненты используется состояние products, которому присваивается результат вызова экшена. 
   * Хорошим решением будет создать computed свойство, которое будет возвращать значение геттера списка товаров.
   * 
   * 11. В компоненте увидел логику с интервалом, возможно того требует задача, чтобы это делать именно тут... Я бы вынес это в стор,
   ...
   * 
   * 13. addToCart() - я бы немного "причесал" этот метод, добавил очистку поля вводу после добавления, сделал бы предупреждение о невведённом
